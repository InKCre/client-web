...
import type ArcEditor from '~/components/info-base/ArcEditor.vue'
...
<template>
  <div :class="['arcs-editor-wrapper', type === 'incoming' ? 'incoming' : 'outgoing']">
    <div ref="container" class="arcs-container" tabindex="0" @keydown="handleKeydown">
      <div
        v-for="(arc, arcIndex) in props.modelValue"
        :key="type + arcIndex.toString()"
        class="arc-item"
      >
        <ArcEditor ref="arcEditors" :modelValue="arc" :type="type" :relationType="relationType" />
      </div>
    </div>
    <div v-if="props.modelValue.length > 1" class="navigation-controls">
      <span class="arc-counter">{{ index + 1 }}/{{ props.modelValue.length }}</span>
      <div class="nav-buttons">
        ...
        <button ref="nextButton" @click="next" class="nav-button next-button">&rarr;</button>
      </div>
    </div>
  </div>
</template>
