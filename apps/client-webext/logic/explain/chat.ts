...
import { explainInstruction } from '../storage'
...
export interface useExplainChatOptions {
  ...
  instructions?: string
  onUpdate?: (update: { messages?: Message[]; isLoading?: boolean; error?: string }) => void
...
export function useExplainChat(options: useExplainChatOptions) {
  ...
  const sendMessage = async (userMessage: string, tabId?: number): Promise<void> => {
    // Add user message
