...
export function useAllCommunitiesLayout(
   ...
   */
  function positionCommunitiesMDS(
    communityBoundsList: CommunityBounds[],
    allEdges: Edge[]
  ): void {
    ...
    const { distances, communityIds } = computeInterCommunityDistances(
      communityMap,
      allEdges
    );
    ...
    const totalArea = communityBoundsList.reduce(
      (sum, cb) => sum + cb.width * cb.height,
      0
    );
    ...
    for (let iter = 0; iter < iterations; iter++) {
      ...
      for (let i = 0; i < communityBoundsList.length; i++) {
        for (let j = i + 1; j < communityBoundsList.length; j++) {
          ...
          const overlapX =
            a.x + a.width + COMMUNITY_GAP > b.x &&
            b.x + b.width + COMMUNITY_GAP > a.x;
          const overlapY =
            a.y + a.height + COMMUNITY_GAP > b.y &&
            b.y + b.height + COMMUNITY_GAP > a.y;
