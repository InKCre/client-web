 ...
 */
function isTree(graph: DirectedGraph, hasCycles: boolean, connected: boolean): boolean {
 ...
 */
function findRootNodes(graph: DirectedGraph, inDegrees: Map<string, number>): string[] {
  const roots: string[] = []
 ...
 */
function findLeafNodes(graph: DirectedGraph, outDegrees: Map<string, number>): string[] {
  const leaves: string[] = []
...
function detectStarCenter(
  ...
  const centerDegree = (inDegrees.get(maxDegreeNode) ?? 0) + (outDegrees.get(maxDegreeNode) ?? 0)
 ...
 */
function computeDepth(graph: DirectedGraph, rootNodes: string[]): number | undefined {
