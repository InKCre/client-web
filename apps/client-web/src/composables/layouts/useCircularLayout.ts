...
import { type CircularLayoutConfig, type LayoutResult, DEFAULT_CIRCULAR_CONFIG } from '@inkcre/core'
...
export function useCircularLayout(options: UseCircularLayoutOptions = {}) {
  ...
  async function apply(nodes: Node[], _edges: Edge[]): Promise<LayoutResult> {
    ...
    try {
      ...
      const radius = mergedConfig.radius > 0 ? mergedConfig.radius : Math.max(100, n * 30)
