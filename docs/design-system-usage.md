...
- **Reference tokens** live in `src/design-system/scss/abstract/_ref.scss` as top-level maps (`$palette`, `$font`, `$space`, `$shape`, `$elevation`, `$opacity`, `$layout`) that expose raw primitives.
- **Typography tokens** are defined in `src/design-system/scss/abstract/_typography.scss` as `$typo-tokens`, hard-encoding sizes/line-heights/weights while reusing the reference font families.
- **Semantic system tokens** reside in `src/design-system/scss/abstract/_semantic.scss` as `$light` and `$dark` maps, producing `--sys-` CSS variables for light and dark themes.
