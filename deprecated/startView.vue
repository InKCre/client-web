    ...
    <main class="workspace__main">
      ...
      <section class="workspace__blocks">
        <header class="workspace__section-header">
          ...
          <button @click="refreshBlocks" :disabled="blocksState.loading" class="workspace__action-btn">
            {{ blocksState.loading ? "LOADING..." : "REFRESH" }}
          </button>
        </header>

        <div class="workspace__blocks-grid" ref="blocksGridRef">
          <BlockViewer v-for="block in blocks" :key="block.id" :block="block" mode="preview" :show-details="false"
            @click="(blockId) => navigateToBlock(blockId)" class="workspace__block-item" />
        </div>
      </section>
