...
- 旧版内置 Discover 已在真实 `registry.inkcre.dev` + 隔离数据库验收；此记录不代表本轮跨站回跳已验收。本轮本地构建页面验证了未连接时的指引、精确 RSS Release 显示和禁用的安装动作；数据库 E2E 已补“回跳不写入、确认后安装”，待 PR CI 使用隔离数据库运行。
- UI 2.1.0 正式包安装后，完整 `pnpm check` 再次通过，包括 Web 与三个 MF Extension 的类型、构建和包边界检查。第三组源码联调阶段已在浏览器检查嵌套/可空字段、未知字段保留、无效 JSON、单一原生 form 和 Tabs 历史；新提交的远端结果待检查。
