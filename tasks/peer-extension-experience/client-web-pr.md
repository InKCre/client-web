...
- Extensions 页面分为默认的 Installed 与独立的 Discover 视图；Peer selector 只控制“在 Peer 上运行”。
- Discover 从 Registry 加载 Extension 列表，支持本地 name/nickname 搜索、按需详情、精确版本与 Core/Web Host 信息。
- 默认选择最高 stable SemVer；仅有 prerelease 时明确标记。安装只写部署级记录，不隐式启用、下载或执行 Extension。
...
- Extensions Installed/Discover 和配置 Form/JSON 复用 UI 包的受控 Tabs；前者保留真实路由链接和浏览器历史。
...
- 真实 `registry.inkcre.dev` + 隔离数据库：浏览、搜索、版本 Host 信息、安装与 Installed 管理态通过
- UI 2.1.0 正式包安装后，完整 `pnpm check` 再次通过，包括 Web 与三个 MF Extension 的类型、构建和包边界检查。第三组源码联调阶段已在浏览器检查嵌套/可空字段、未知字段保留、无效 JSON、单一原生 form 和 Tabs 历史；最新提交的远端 Workspace contract、Web E2E、Webext E2E 与 Cloudflare preview 均通过。
...
- 同一联网环境完成导出→隔离浏览器导入→恢复同一 Peer 身份与在线列表；当前浏览器 Peer 人工改名在刷新后保留且已恢复原名。Registry 搜索、版本分发标识、RSS 一键安装→Core 启用→Source 可视化创建/编辑/刷新→删除→扩展停用/卸载均通过。JSON 无效草稿和未知键的处理、Core/Web Peer 表单、密码遮蔽、Tabs 历史也已复核；Core Peer 配置无变更保存后刷新仍完整。未运行第三方采集；验收临时记录与含密钥导出文件已清理。
- 仍待补一条不预填 localStorage 的 Settings 保存→刷新→Peers E2E；未连接浏览器在 Discover 显示底层 `Invalid URL` 属后续交互质量分组，不能混入本轮通过范围。

关联：InKCre/ext-reg#48、InKCre/core-py#118、InKCre/ui#50、InKCre/ui#51、InKCre/core-py#120、InKCre/core-py#121、InKCre/docs#31。
