...
启动修复前两轮 CI：原有 12 条通过，新增旅程在网络断开或 503 后 5 秒内未见错误态。锁定的 PostgREST SDK 2.110.8 对 GET 网络失败及 520/503 均按 1/2/4 秒退避重试；第二轮 trace 显示同一 Peer GET 三次返回 503、重试计数 1/2，页面保持三点式等待。首次判断仅看了 SDK 函数注释而漏查实际状态码清单，错误地把 503 当作终态。最终改为受控 401，验证凭据被拒绝后的恢复边界，不修改 SDK、不延长断言超时。公开新版 preview 已实测三点式启动提示 → RSS 0.2.1 精确版本页；本地不可达连接已实测失败 → Retry → Settings，配置仍保留。新版完整 CI 待通过。
