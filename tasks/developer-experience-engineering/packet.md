...
- **Current Truth**: client-web is a static Vue SPA whose environment configuration remains browser-owned. Main `8d35299` consumes core-py's `stable` channel, generates relation types from its image-carried raw schema, and validates the same immutable service against fresh PostgreSQL and PostgREST. Pull-request validation, trusted same-repository preview delivery, exact-main focused production release, and exact preview cleanup are implemented as separate controllers. Temporary compatibility contexts keep the previous branch-protection names valid until the settings cutover. Client-web does not own migrations, role SQL, seed ordering, or core release admission. See [evidence](./10-evidence.md).
