...
- **Current Truth**: client-web builds and runs as a static Vue SPA with no Hono, application Worker, Wrangler config, or runtime config endpoint. Web bootstrap config is validated and stored only in the browser origin; webext initializes the same core store from extension-owned storage. The user-supplied JWT signing credential is masked, never supplied through Vite or Cloudflare, and excluded from portable config exports. PDM global supplies the published SVC 10.0.1 wheel. SVC and Portless expose worktree-scoped `web`, `webext`, and database capabilities with exact instance probes and bounded cleanup. The database capability consumes core-py's digest-pinned OCI runtime and versioned `inkcre` protocol contract through one provider-neutral Compose lifecycle: committed local Docker is the portable default, while ignored machine configuration may select a validated SSH alias, remote dynamic ports, and instance-owned local tunnels. Client-web does not own migrations, role SQL, seed ordering, or reset semantics. Generated relation types, profile/JWT drift checks, deterministic browser/database E2E, legacy endpoint migration, and trusted static Pages controllers are implemented locally. One root Vitest 4 project graph owns core, web, webext, and runtime tests; Playwright separately loads the exact Chromium extension artifact and exercises the peer database browser path. Required GitHub checks cover workspace, dependency review, peer database, and browser extension; shadow lanes, Firefox output, failure evidence, immutable Action SHAs, and dependency updates are published. The package-manager contract hard-cuts to pnpm 11.11.0 and explicit dependency-build policy; the high-severity audit keeps pnpm's official analyzer behind a bounded transport adapter for npm's current missing gzip response header. Cloudflare Pages production and internal-PR preview controllers consume the exact accepted artifact, isolate Wrangler from the private pnpm workspace, and pass root plus SPA fallback smoke. Direct Upload project `inkcre-client-web`, production branch `main`, custom domain `app.inkcre.dev`, proxied CNAME, repository-scoped organization secrets, and project variable are active. Phase 2 was committed as `6902293`, the static runtime baseline as `5da90de`, the generic provider follow-up as `7d4f07a`, and the test/CD ratchets as `74c1f72`. See [evidence](./10-evidence.md).
- **Next Step**: prove preview deletion when an eligible internal PR closes, ratchet the remaining low advisory and action-runtime warnings, then decide the hard cut for long-lived `develop` and legacy staging control-plane state.
