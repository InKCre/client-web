...
- **Current Truth**: client-web builds and runs as a static Vue SPA with no Hono, application Worker, Wrangler config, or runtime config endpoint. Web bootstrap config is validated and stored only in the browser origin; webext initializes the same core store from extension-owned storage. The user-supplied JWT signing credential is masked, never supplied through Vite or Cloudflare, and excluded from portable config exports. PDM global supplies the published SVC 10.0.1 wheel. SVC and Portless expose worktree-scoped `web`, `webext`, and database capabilities with exact instance probes and bounded cleanup. The database capability consumes core-py's digest-pinned OCI runtime and versioned `inkcre` protocol contract through one provider-neutral Compose lifecycle: committed local Docker is the portable default, while ignored machine configuration may select a validated SSH alias, remote dynamic ports, and instance-owned local tunnels. Client-web does not own migrations, role SQL, seed ordering, or reset semantics. Generated relation types, profile/JWT drift checks, deterministic browser/database E2E, legacy endpoint migration, and trusted static Pages controllers are implemented locally. One root Vitest 4 project graph now owns core, web, webext, and runtime tests; Playwright separately loads the exact Chromium extension artifact and exercises the peer database browser path. CI ratchets for those suites, Firefox output, failure evidence, immutable Action SHAs, dependency updates, and post-deploy Pages smoke are published. The package-manager contract hard-cuts to pnpm 11.11.0 and explicit dependency-build policy; the high-severity audit keeps pnpm's official analyzer behind a bounded transport adapter for npm's current missing gzip response header. Wrangler OAuth is healthy through the pinned `pnpm dlx` invocation. The Direct Upload project `inkcre-client-web` uses production branch `main`; `app.inkcre.dev` and its proxied CNAME are active. Repository-scoped organization secrets and the project variable are installed for `InKCre/client-web`, but the project has no canonical deployment until the audit repair publishes and the controller succeeds. Phase 2 was committed as `6902293`, the static runtime baseline as `5da90de`, the generic provider follow-up as `7d4f07a`, and the test/CD ratchets as `74c1f72`. See [evidence](./10-evidence.md).
- **Next Step**: publish the audit hard cut, prove the exact GitHub artifact through production Pages smoke, add the browser-extension check to protected `main`, and prove one per-PR preview lifecycle.
