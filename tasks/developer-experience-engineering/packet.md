...
- Phase 6 is locally implemented and verified. The user authorized the environment-neutral hard
  cut and Dependabot convergence, but not the bootstrap/subscription implementation and not a
  commit or push. Dependabot publication is additionally gated by the manually created
  `INKCRE_PACKAGES_READ_TOKEN` secret.
